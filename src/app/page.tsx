...
export default function HomePage() {
  return (
    <main className="min-h-screen bg-gradient-to-b from-drcBlue to-slate-900 text-white p-8">
      <header className="max-w-6xl mx-auto text-center my-12">
        <span className="bg-drcYellow text-slate-900 px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-wider">
          Kinshasa Portal
        </span>
        <h1 className="text-5xl font-black mt-4 uppercase tracking-tight">
          Kinshasa <span className="text-drcYellow">Flag</span> Interactive Map
        </h1>
        ...
        <div className="mt-6">
          <Link href="/login" className="bg-drcRed text-white px-5 py-2 rounded-lg text-xs font-bold hover:opacity-90 transition">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {KINSHASA_COMMUNES.map((commune) => (
            <Link 
              ...
              className="p-4 rounded-xl border border-slate-700 bg-slate-800 bg-opacity-50 text-center font-semibold hover:border-drcYellow hover:scale-[1.02] transition-all"
            >
              <p className="hover:text-drcYellow">{commune}</p>
            </Link>
          ))}
        </div>
      </section>
