...
export default function CommuneHubPage({ params }: { params: { name: string } }) {
  ...
  return (
    <main style={{ backgroundColor: '#020617', color: '#f8fafc', minHeight: '100vh', padding: '16px', fontFamily: 'system-ui, sans-serif' }}>
      ...
      {/* Header */}
      <nav style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', maxWidth: '1650px', margin: '0 auto 20px auto', borderBottom: '1px solid #1e293b', paddingBottom: '12px', flexWrap: 'wrap', gap: '10px' }}>
        <div>
          <span style={{ fontSize: '10px', color: '#38bdf8', fontWeight: 'bold', textTransform: 'uppercase' }}>KINSHASA LABEL — Média-Guide</span>
          <h1 style={{ fontSize: '20px', fontWeight: 'bold', color: '#ffffff', margin: 0 }}>{communeName} Sector Guide</h1>
        </div>
        <Link href="/" style={{ backgroundColor: '#dc2626', color: '#ffffff', padding: '8px 14px', borderRadius: '8px', textDecoration: 'none', fontWeight: 'bold', fontSize: '11px' }}>← Carte</Link>
      </nav>

      <div style={{ maxWidth: '1650px', margin: '0 auto' }}>
        
        {/* Banner */}
        <div style={{ backgroundColor: '#0f172a', border: '1px solid #1e293b', borderRadius: '14px', padding: '20px', marginBottom: '20px' }}>
          <span style={{ color: '#22c55e', fontSize: '10px', fontWeight: 'bold', textTransform: 'uppercase' }}>★ CERTIFIÉ KINSHASA LABEL</span>
          <h1 style={{ fontSize: '28px', fontWeight: '900', color: '#ffffff', margin: '4px 0 0 0', textTransform: 'uppercase' }}>
        ...
        <section style={{ backgroundColor: '#0f172a', border: '1px solid #a855f7', borderRadius: '14px', padding: '18px', marginBottom: '20px' }}>
          ...
          <div className="commune-grid">
            {events.length === 0 ? <p style={{ fontSize: '11px', color: '#64748b' }}>Aucun événement ce weekend.</p> : events.map(e => (
        ...
        <div className="commune-grid">
          <section style={{ backgroundColor: '#0f172a', border: '1px solid #1e293b', borderRadius: '14px', padding: '18px' }}>
            <h2 style={{ fontSize: '16px', color: '#22c55e', margin: '0 0 12px 0', textTransform: 'uppercase' }}>🍔 KIN FOOD ({foodPlaces.length})</h2>
            {foodPlaces.length === 0 ? <p style={{ fontSize: '11px', color: '#64748b' }}>Aucune adresse.</p> : foodPlaces.map(renderPlaceCard)}
          </section>

          <section style={{ backgroundColor: '#0f172a', border: '1px solid #1e293b', borderRadius: '14px', padding: '18px' }}>
            <h2 style={{ fontSize: '16px', color: '#38bdf8', margin: '0 0 12px 0', textTransform: 'uppercase' }}>📍 KIN PLACES ({otherPlaces.length})</h2>
            {otherPlaces.length === 0 ? <p style={{ fontSize: '11px', color: '#64748b' }}>Aucun lieu.</p> : otherPlaces.map(renderPlaceCard)}
          </section>

          <section style={{ backgroundColor: '#0f172a', border: '1px solid #1e293b', borderRadius: '14px', padding: '18px' }}>
            <h2 style={{ fontSize: '16px', color: '#f59e0b', margin: '0 0 12px 0', textTransform: 'uppercase' }}>🎨 KIN CULTURE ({culturePlaces.length})</h2>
            {culturePlaces.length === 0 ? <p style={{ fontSize: '11px', color: '#64748b' }}>Aucun espace.</p> : culturePlaces.map(renderPlaceCard)}
          </section>

          <section style={{ backgroundColor: '#0f172a', border: '1px solid #1e293b', borderRadius: '14px', padding: '18px' }}>
            <h2 style={{ fontSize: '16px', color: '#ec4899', margin: '0 0 12px 0', textTransform: 'uppercase' }}>👗 KIN STYLE ({stylePlaces.length})</h2>
            {stylePlaces.length === 0 ? <p style={{ fontSize: '11px', color: '#64748b' }}>Aucun créateur.</p> : stylePlaces.map(renderPlaceCard)}
          </section>
        </div>
      </div>
    </main>
