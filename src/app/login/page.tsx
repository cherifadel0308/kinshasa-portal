...
export default function LoginPage() {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-slate-900 px-4">
      <form onSubmit={handleLogin} className="max-w-md w-full bg-white p-8 rounded-2xl shadow-xl space-y-4 border-t-8 border-drcBlue">
        ...
        {errorMsg && <p className="text-xs text-drcRed font-bold">{errorMsg}</p>}
        <button type="submit" className="w-full bg-drcBlue text-white p-2.5 rounded font-bold">Sign In</button>
      </form>
    </main>
  );
}
